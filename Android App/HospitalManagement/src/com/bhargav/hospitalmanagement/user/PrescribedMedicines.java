...
public class PrescribedMedicines extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		title.setText("My Prescribed Medicines");

		final ListView listview = (ListView) findViewById(R.id.list_data);

		final QueryExecutor QE = new QueryExecutor(PrescribedMedicines.this);
		String result = null;
		result = QE.GetPrescribedMedicines(UserId);
		...
		{
			String Dosage_TimePrint = "";
			try {
				for(int k=1;k<=Integer.parseInt(jsons[i].getString("Dosage"));k++)
				{
					Dosage_TimePrint += "Time-" + k + " :  " + jsons[i].getString("Time" + k) + " hours\n";
				}
			...
			try {
				list.add(     "Doctor Name :" 			+ jsons[i].getString("DoctorName") 
						+ "\nPharmacy Name : "          + jsons[i].getString("PharmacyName")
						+ "\nMedicine Name : "          + jsons[i].getString("MedicineName")
						+ "\nDosage : "          	+ jsons[i].getString("Dosage") + " times a day\n"
						+ Dosage_TimePrint
						+ "Disease : "          	+ jsons[i].getString("PatientDisease")
						+"\n\n\n");
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
